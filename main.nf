...
process COLOC {
    ...
    tuple path(input_script), path(sumstat1), path(sumstat2), path(ld_matrix)
    ...
    script:
    """
    echo "[INFO] Running the R script with 3 input files!"
    Rscript ${input_script} ${sumstat1} ${sumstat2} ${ld_matrix}

    echo "[INFO] R script execution completed."
    """
}
// TOO HERE
...
workflow mypipeline {
  ...
  // Show help message
    ...
    ch_input_files = Channel.of(
        tuple(
            file(params.input),
            file(params.sumstat1),
            file(params.sumstat2),
            file(params.ld_matrix)
        )
    )
    // TOO HERE
